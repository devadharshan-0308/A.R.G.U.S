      ...
      const popupContent = `
        <div style="font-family: 'Inter', sans-serif; min-width: 180px; padding: 2px;">
          <div style="display: flex; align-items: center; justify-content: space-between; gap: 8px; margin-bottom: 4px;">
            ...
            <span style="font-family: monospace; font-size: 0.7rem; color: #94a3b8;">${ev.timestamp_sec ? ev.timestamp_sec.toFixed(1) + 's' : ''}</span>
          </div>
          <div style="font-size: 0.75rem; color: #e2e8f0; line-height: 1.3;">
            ${ev.formatted_address || ev.street_name || 'MG Road Corridor, Bengaluru'}
          </div>
          ${ev.is_school_zone ? `<span style="display: inline-block; margin-top: 4px; font-size: 0.65rem; background: rgba(168,85,247,0.2); color: #c084fc; padding: 1px 5px; border-radius: 4px; border: 1px solid rgba(168,85,247,0.4);">🏫 School Zone</span>` : ''}
          ${ev.is_hospital_zone ? `<span style="display: inline-block; margin-top: 4px; font-size: 0.65rem; background: rgba(239,68,68,0.2); color: #f87171; padding: 1px 5px; border-radius: 4px; border: 1px solid rgba(239,68,68,0.4);">🏥 Hospital Zone</span>` : ''}
          ${evidenceHtml}
        </div>
      `;
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <span style={{ color: '#38bdf8' }}>{events.length} active markers</span>
      </div>
    </div>
  );
}
