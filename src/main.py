...
def main(args=None):
    ...
    # 1. Ensure output directories and SQLite database exist
    os.makedirs(os.path.dirname(args.output), exist_ok=True)
    os.makedirs(os.path.dirname(args.json_output), exist_ok=True)
    db.init_db()

    logger.info("=" * 60)
    logger.info("  A.R.G.U.S. - Autonomous Roadway Governance & Urban Sensing")
    logger.info("  Full-Stack Edge AI Ingestion & Analysis Pipeline")
    logger.info("=" * 60)
    ...
    route_sim = RouteSimulator(video_path=args.input)
    cap_probe = cv2.VideoCapture(args.input)
    total_video_frames = int(cap_probe.get(cv2.CAP_PROP_FRAME_COUNT))
    cap_probe.release()
    if total_video_frames <= 0:
        total_video_frames = 600

    _last_pothole_event = None
    _pushed_infra_cooldown = {}
    all_distress_incidents = []
    ...
    try:
        for packet in video_frame_generator(args.input, frame_skip=args.skip):
            ...
            raw_fid = packet.get("raw_frame_id", frame_id)
            progress_frac = min(1.0, raw_fid / total_video_frames) if total_video_frames > 0 else 0.0
            sim_lat, sim_lon, sim_heading, sim_street = route_sim.get_position(progress_frac)

            if "latitude" in packet and "longitude" in packet:
                gps_lat, gps_lon = packet["latitude"], packet["longitude"]
            else:
                gps_lat, gps_lon = sim_lat, sim_lon

            loc_info = maps_enricher.enrich_location(gps_lat, gps_lon) if maps_enricher else {}
            street_name = loc_info.get("street_name") or sim_street
            ...
            if pothole_detector is not None:
                ...
                if pothole_event is not None:
                    ...
                    for p_det in potholes_detected:
                        ...
                        # If newly discovered incident, record to PWD distress collection & SQLite DB
                        if is_new:
                            p_entry = {
                                "type": "POTHOLE",
                                "id": f"POT-{incident.pothole_id}",
                                "title": f"{p_class.title()}",
                                "severity": "CRITICAL" if "severe" in p_class else ("WARNING" if "mild" in p_class else "INFO"),
                                "gps": f"{gps_lat:.5f}, {gps_lon:.5f}",
                                "lat": gps_lat,
                                "lon": gps_lon,
                                "confidence": f"{p_conf*100:.0f}%",
                                "location": street_name
                            }
                            all_distress_incidents.append(p_entry)
                            try:
                                db.insert_pothole({
                                    "incident_id": p_entry["id"],
                                    "frame_id": frame_id,
                                    "timestamp_sec": ts,
                                    "latitude": gps_lat,
                                    "longitude": gps_lon,
            ...
            # Run BEFORE plate detection so Rule C (rash driving / hit-and-run)
            # can decide WHICH vehicles are worth OCR'ing.
            rule_result = rule_engine.evaluate(traffic_event, image=packet["image"], location_context=loc_info)
            frame_violations = rule_result.get("violations", [])
            violation_track_ids = {v["track_id"] for v in frame_violations}
            for v in frame_violations:
                try:
                    db.insert_violation({
                        ...
                        "longitude": gps_lon,
                        "street_name": street_name,
                        "violation_type": v.get("rule", "RULE_VIOLATION"),
            ...
            # Step 3b: Run Local License Plate Detector & OCR — only when
            # Rule C has flagged at least one vehicle as rash-driving / hit-and-run.
            # This is incident-triggered ANPR instead of scanning every frame.
            detected_plates = []
            if plate_detector is not None and violation_track_ids:
                ...
                for pl in detected_plates:
                    try:
                        db.insert_plate({
                            ...
                            "confidence": pl.get("confidence", 0.9)
                        })
                    except Exception:
                        pass
            ...
            # Record infrastructure deficiencies to PWD collector & SQLite
            for d in infra_defects:
                d_type = d.get("type", "defect")
                d_sev = d.get("severity", "WARNING")
                if d_sev != "INFO" and d_type not in ("road_divider", "zebra_crossing", "signboard"):
                    last_push = _pushed_infra_cooldown.get(d_type, -999.0)
                    if (ts - last_push) >= 2.5:
                        _pushed_infra_cooldown[d_type] = ts
                        inf_entry = {
                            "type": d_type.upper(),
                            "id": f"INFRA-{d_type[:6]}-{frame_id}",
                            "title": d.get("label", "Infrastructure Deficiency"),
                            "severity": d_sev,
                            "gps": f"{gps_lat:.5f}, {gps_lon:.5f}",
                            "lat": gps_lat,
                            "lon": gps_lon,
                            "confidence": f"{d.get('confidence', 0.85)*100:.0f}%",
                            "location": street_name
                        }
                        all_distress_incidents.append(inf_entry)
                        try:
                            db.insert_violation({
                                "incident_id": inf_entry["id"],
                                "frame_id": frame_id,
                                "timestamp_sec": ts,
                                "latitude": gps_lat,
                                "longitude": gps_lon,
                                "street_name": street_name,
                                "violation_type": d_type.upper(),
                                "severity": d_sev,
                                "description": d.get("label", "Infrastructure Deficiency")
                            })
                        except Exception:
                            pass
        ...
        if all_distress_incidents:
            try:
                from src.pwd_workorder import generate_pwd_work_orders
                csv_file, pwd_sum = generate_pwd_work_orders(all_distress_incidents)
                pwd_summary_info = (csv_file, pwd_sum)
                for ord_item in pwd_sum.get("work_orders", []):
                    try:
                        db.insert_pwd_work_order(ord_item)
                    except Exception:
                        pass
        ...
        try:
            db.insert_audit_run({
                ...
                "total_potholes": spatial_deduplicator.get_summary()["total_unique_potholes"] if spatial_deduplicator else len(all_distress_incidents),
                "total_vehicles": total_vehs if 'total_vehs' in locals() else 0,
                "total_pedestrians_tracked": peds_count if 'peds_count' in locals() else 0,
                "total_pwd_orders": pwd_summary_info[1].get("total_orders", 0) if pwd_summary_info else 0,
                "total_pwd_budget_inr": pwd_summary_info[1].get("total_budget_inr", 0.0) if pwd_summary_info else 0.0
